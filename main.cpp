	...
	for (int i = 0; i < orderedTask.size(); i++) {
		...
		if (expectedMachine1End < getCmax(machine2Blocks)) {
			...
			if (noWaitBreak >= maintenance) {
				if (getCmax(machine2Blocks) - maintenance >= lastMaintenance(machine1Blocks) + maxTimeWork) {
					...
					noWaitBreak = getCmax(machine2Blocks) - getCmax(machine1Blocks);
				}
				else {
					addBlock(machine1Blocks, getCmax(machine2Blocks) - maintenance, getCmax(machine2Blocks), "maintenance");
					noWaitBreak = 0;
				}
			}
			...
			int machine1End = getCmax(machine1Blocks);
			...
			if (noWaitBreak >= maintenance && machine1End != 0) {
				if (getCmax(machine1Blocks) - maintenance >= lastMaintenance(machine2Blocks) + maxTimeWork) {
					...
					noWaitBreak = machine1End - getCmax(machine2Blocks);
				}
				else {
					addBlock(machine2Blocks, getCmax(machine1Blocks) - maintenance, getCmax(machine1Blocks), "maintenance");
					noWaitBreak = 0;
				}
			}
			addBlock(machine2Blocks, getCmax(machine1Blocks), getCmax(machine1Blocks) + tasks[orderedTask[i]].get_op2(), "task", tasks[orderedTask[i]].get_task_number(), 2, maintenance, tasks[orderedTask[i]].get_op2(), maxTimeWork, latency);
...
void tabuSearch(std::vector <Task> &tasks, std::vector <MachineBlock> &machine1Blocks, std::vector <MachineBlock> &machine2Blocks, const int maintenance, const int maxTimeWork, const float latency, int neighborhood, int newInitialSolutionCount) {
	...
	while (newInitialSolutionCount != 0) {
		...
		int improvementCount = ceil(0.5 * tasks.size());
		int neighborhoodIndex = neighborhood;
		while (neighborhoodIndex > 0) {
